<?php
    ...
    $verificarAdministrador = $conn->prepare("SELECT a.idAdministrador 
                                       FROM administrador a 
                                       WHERE a.correoInstitucional = ?");
    $verificarAdministrador->bind_param("s", $correoAdministrador);
    $verificarAdministrador->execute();
    $verificarAdministrador->bind_result($idAdministrador);
    $verificarAdministrador->fetch();
    $verificarAdministrador->close();

    // Filtro para evitar que el administrador actual interactúe con su cuenta de administrador
    $administradores = $conn->prepare("SELECT a.idAdministrador, 
    ...
    $administradores->bind_param("i", $idAdministrador);
    $administradores->execute();
    $result = $administradores->get_result();
...
<body>
    ...
    <div class="new-button-container">
        <button class="buttonNew" onclick="location.href = './registroAdministrador.php' "><i class="fas fa-plus"></i> Nuevo</button>
    </div>
                        ...
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<tr>
                                    ...
                                    <td class='action-buttons autoTable'>
                                        ...
                                        <button class='delete' data-id-administrador='{$idAdministrador}' data-csrf-token='{$_SESSION['csrf_token']}'><i class='fas fa-trash-alt'></i></button>
                                    </td>
                                </tr>";
                        }
                    }
