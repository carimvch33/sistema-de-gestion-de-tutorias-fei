...
<head>
    ...
    <link rel="stylesheet" href="<?= BASE_URL; ?>/assets/css/administrarEstudiantes.css">
...
<body>
    ...
    <div class="new-button-container">
        <button class="buttonNew" onclick="location.href = '<?= BASE_URL; ?>/registroEstudiante.php' "><i class="fas fa-plus"></i> Nuevo</button>
    </div>
    ...
    <div class="table-container">
        <table id="estudiantesTable">
            <thead>
                <tr>
                    <th class="autoWidthColumn">Nombre de estudiante</th>
                    <th class="autoWidthColumn">Matrícula</th>
            ...
            <tbody>
                ...
                if (!empty($estudiantes)) {
                    foreach ($estudiantes as $row) {
                        $idTutorado = $row['idTutorado'];
                        echo "<tr>
                                <td>{$row['tutoradoNombre']}</td>
                                <td>{$row['matricula']}</td>
                                <td>{$row['carrera']}</td>
                                <td>{$row['correoInstitucional']}</td>
                                <td>{$row['tutorNombre']}</td>
                                <td class='action-buttons autoTable'>
                                    <button class='edit' data-id-tutorado='{$idTutorado}'><i class='fas fa-edit'></i></button>
                                    <button class='delete' data-id-tutorado='{$idTutorado}' data-csrf-token='{$csrf_token}'><i class='fas fa-trash-alt'></i></button>
                                </td>
                            </tr>";
                    }
                }
    ...
    <script src="<?= BASE_URL; ?>/assets/js/administrarEstudiantes.js"></script>
</body>
