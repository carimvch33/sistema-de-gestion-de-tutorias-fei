<?php
    ...
    if($idAdministrador){
        ...
        if ($correoAdministrador === $correoActual) {
            ...
            $sesionBusqueda->bind_param("i", $idAdministrador);
            ...
            if($idSesion){
                ...
                if($sesionEliminar->execute()){
                    $profesorEliminar = $conn->prepare("DELETE FROM administrador a WHERE a.idAdministrador = ?");
                    $profesorEliminar->bind_param("i", $idAdministrador);
        ...
        echo json_encode(['status' => 'error', 'message' => 'Error al obtener ID administrador.']);
    }
